...
> <det>
    ...
    def _export_fields_nfe_40_icms(self, xsd_fields, class_obj, export_dict):
        ...
        if not self.nfe40_choice_icms:
            raise UserError(
                _(
                    "Não foi possível determinar o grupo de tributação do ICMS "
                    "para a linha do produto '%(product)s' (CST/CSOSN: '%(cst)s'). "
                    "Verifique se a Operação Fiscal da linha possui um CST/CSOSN "
                    "de ICMS válido configurado."
                )
                % {
                    "product": self.product_id.display_name,
                    "cst": self.icms_cst_id.code or _("não definido"),
                }
            )
